...
import { Activity, AlertTriangle, CheckCircle2, Compass, FileWarning, HeartHandshake, MapPin, Phone, Shield, Siren, Sparkles, Users } from "lucide-react"
...
function useDemoUser() {
  ...
  return routeUser ?? legacyUser
}

export function TouristDashboardView() {
  const user = useDemoUser()
  const [currentLocation, setCurrentLocation] = useState<LocationData | null>(null)
  const handleLocationDetected = useCallback((location: LocationData) => setCurrentLocation(location), [])
  if (!user) return null
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      <section className="grid gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)]">
        ...
        <Card><CardHeader><CardDescription className="font-medium uppercase tracking-[0.16em]">Current location</CardDescription><CardTitle className="flex items-center gap-2 text-2xl"><MapPin className="h-5 w-5 text-emerald-600" /> Browser GPS</CardTitle></CardHeader><CardContent className="space-y-4"><LocationStatus onLocationDetected={handleLocationDetected} /><Link href="/location" className="inline-flex items-center gap-2 text-sm font-semibold text-primary hover:underline"><Compass className="h-4 w-4" /> Open full location tools</Link></CardContent></Card>
      </section>

      <section className="grid gap-6 xl:grid-cols-[minmax(0,1.25fr)_minmax(300px,0.75fr)]"><Card className="overflow-hidden"><CardHeader className="flex flex-row items-center justify-between space-y-0"><div><CardTitle className="flex items-center gap-2"><MapPin className="h-5 w-5 text-primary" /> Safety map</CardTitle><CardDescription>{currentLocation ? "Your detected browser location" : "Allow location access to center the map"}</CardDescription></div><Badge className="bg-emerald-100 text-emerald-800 hover:bg-emerald-100">Demo map</Badge></CardHeader><CardContent className="p-0"><div className="h-[270px] sm:h-[330px]">{currentLocation ? <InteractiveMap onLocationSelect={() => undefined} currentLocation={{ lat: currentLocation.latitude, lng: currentLocation.longitude }} safetyZone="green" /> : <div className="flex h-full items-center justify-center p-6 text-center text-sm text-muted-foreground">Your real browser location will appear here after permission is granted.</div>}</div></CardContent></Card><Card><CardHeader><CardTitle>Safety zones</CardTitle><CardDescription>How SafeVoyage labels trip conditions</CardDescription></CardHeader><CardContent className="space-y-4">{[{ label: "Safe zone", note: "Low concern", color: "bg-emerald-500", icon: CheckCircle2 }, { label: "Caution zone", note: "Stay alert", color: "bg-amber-400", icon: AlertTriangle }, { label: "Risk / danger", note: "Take action", color: "bg-rose-500", icon: Siren }].map((zone) => { const Icon = zone.icon; return <div key={zone.label} className="flex items-center gap-3"><span className={`flex h-9 w-9 items-center justify-center rounded-full ${zone.color} text-white`}><Icon className="h-4 w-4" /></span><div><p className="text-sm font-semibold">{zone.label}</p><p className="text-xs text-muted-foreground">{zone.note}</p></div></div> })}<p className="border-t pt-4 text-xs leading-5 text-muted-foreground">Zone labels are simulated for this demo and should not replace local guidance or emergency services.</p></CardContent></Card></section>
      ...
      <Card className="border-sky-200 bg-sky-50/70"><CardContent className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-sky-100 text-sky-700"><Sparkles className="h-5 w-5" /></div><div><p className="font-semibold text-sky-950">Need a quick safety answer?</p><p className="mt-1 text-sm text-sky-800">Ask the SafeVoyage Safety Assistant about feeling unsafe, SOS, lost phones, or incident reports.</p></div></div><Button asChild variant="outline" className="shrink-0 border-sky-300 bg-white"><Link href="/assistant">Open Safety Assistant</Link></Button></CardContent></Card>
